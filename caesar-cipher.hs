-- Code copied from the book Programming in Haskell 2nd Edition
import Data.Char (chr, isLetter, isLower, isUpper, ord, toLower, toUpper)
...
shift' :: Int -> Char -> Char
shift' n c
  | isLetter c = int2char' ((char2int' c + n) `mod` 26)
  | otherwise = c

encode' :: Int -> String -> String
encode' n cs =
  [ if isUpper c then toUpper c' else c'
    | (c, c') <- zip cs (map (shift' n . toLower) cs)
  ]